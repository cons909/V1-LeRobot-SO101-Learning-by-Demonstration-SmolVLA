...
| **2 cameras** ("side left + side right") | One left-side camera + one right-side camera | 80 | 50,000 | Internally the checkpoint's input feature keys are literally named `front`/`side` (that's what the trained model expects — a code-level detail, not a placement description). Started as a claw-mounted camera + one side camera; the claw camera was permanently swapped out mid-project for a second side camera, so this became genuinely two side views. |
| **Side + claw** | One left-side camera + one claw-mounted camera | 80 | 20,000 | Subjectively the most capable of the three during earlier hand-testing, despite added latency from the claw camera's mount. |
...
<div align="center">
<img src="media/charts and images/outcome_comparison.png" alt="Trial outcomes by camera configuration, all 8 graded categories, N=50 per model" width="720">
</div>
...
vs. how claw + side had more green and less red. Full definitions for every category are in
[`docs/EVALUATION_PROTOCOL.md`](docs/EVALUATION_PROTOCOL.md).
...
- **Overfitting to the 80 training demonstrations.** V1 has no held-out validation split —
  ...
  generalizing, and 50k steps is more opportunity for that to happen than 20k.
...
1. **Set up the environment (one-time):** `conda create -n smolvla python=3.10 && conda activate
   smolvla`, then install LeRobot and its SmolVLA extras into that environment per the
   [LeRobot install guide](https://github.com/huggingface/lerobot).
2. **Training:** each checkpoint was fine-tuned from the pretrained SmolVLA base on the Mac
   (`--policy.device=mps`, not the Pi), e.g. `lerobot-train --dataset.repo_id=<dataset>
   --policy.type=smolvla --policy.pretrained_path=lerobot/smolvla_base --policy.device=mps
   --steps=<20000 or 50000 — see the table above> --batch_size=4 --policy.push_to_hub=false`.
   Only needed to reproduce training itself, not to run eval against an existing checkpoint.
3. **Mac side:** `conda activate smolvla`, then set `PI_IP` at the top of
...
6. **Regenerate the results table:** `python harness/summarize_eval.py --all` from inside this
